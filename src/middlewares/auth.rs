...
pub async fn auth<'a, B>( State(app_state): State<Arc<AppState>>, Extension(cookies): Extension<Cookies>, request: Request<B>, next: Next<B>) -> Result<Response, StatusCode> {
    info!("{:?}", cookies);
...
pub async fn authBearer<'a, B>( State(app_state): State<Arc<AppState>>, headers: HeaderMap, request: Request<B>, next: Next<B>) -> Result<Response, StatusCode> {
