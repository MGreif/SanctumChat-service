...
pub async fn get_friends_for_user_from_db(pool: &mut PooledConnection<ConnectionManager<PgConnection>>, client_uuid: &String) -> Vec<UserDTO> {
    let query = diesel::sql_query("SELECT users.username as username, users.public_key FROM friends as f LEFT JOIN users ON f.befriended_user_id = users.username WHERE f.user_id = $1")
