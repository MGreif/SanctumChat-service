use diesel::{r2d2::{PooledConnection, ConnectionManager}, PgConnection, sql_types::{Text, Bool, Uuid}};
use diesel::prelude::*;
use diesel::query_dsl::*;
use crate::{schema::{users::dsl::*, friend_requests::{self}}, handler::friend_handler::FriendRequestGETResponseDTO, models::FriendRequest};
...
impl FriendRequestRepositoryInterface for FriendRequestRepository {
    ...
    fn save_friend_request(&mut self, friend_request: FriendRequest) -> Result<(), String> {
        let inserted_rows = match diesel::insert_into(friend_requests::table).values(&vec![friend_request]).execute(&mut self.pg_pool) {
            Ok(t) => t,
